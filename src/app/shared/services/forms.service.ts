...
export class FormsService {
  ...
  getRegionByCity(city: string) {
    return this.http.get(
      environment.API_URL + this.auth + this.regione + `/${city}`,
      { responseType: 'text' }
    );
  }
  TlogIn(body: any) {
  ...
  verifyAziendaToken(token: string) {
    return this.http.get(
      environment.API_URL + this.auth + this.azAccess + `/${token}`
    );
  }
  verifyTrasportatoreToken(token: string) {
    return this.http.get(
      environment.API_URL + this.auth + this.tAccess + `/${token}`
    );
  }
  verifyAziendaRToken(token: string) {
    return this.http.get(
      environment.API_URL + this.auth + this.azRefresh + `/${token}`
    );
  }
  verifyTrasportatoreRToken(token: string) {
    return this.http.get(
      environment.API_URL + this.auth + this.tRefresh + `/${token}`
    );
  }
  verifyEmail(email: any) {
    return this.http.post(
      environment.API_URL + this.auth + this.resetPassword,
      email
    );
  }
  verifyCode(code: any) {
    return this.http.post(
      environment.API_URL + this.auth + this.testSecretCode,
      code
    );
  }
